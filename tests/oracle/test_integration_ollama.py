"""Opt-in integration tests against a live Ollama daemon (native API).
...
CONTRIBUTING.md. These cover the native ``/api/chat`` adapter only; any
OpenAI-compatible server (including Ollama's ``/v1``) is exercised by
``test_integration_openai_compat.py``. Smoke tests of transport and
parsing against a real server, never model quality.
"""
